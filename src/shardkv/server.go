...
func (kv *ShardKV) run() {
	for {
		...
		case msg := <-kv.applyCh:
			...
			case newSend:
				//判断集群是否负责这个shard
				if !kv.haveShard(op.Shard) && kv.shardInDB(op.Shard) {
			...
			case request:
				...
				} else if !kv.haveShard(op.Shard) || !kv.shardInDB(op.Shard) {
					//本集群不再负责这个shard
...
//数据库里面有没有这里
func (kv *ShardKV) shardInDB(shard int) bool {
	//判断这个shard收到没有
	_, ok := kv.skvDB[shard]
	return ok
}
...
func (kv *ShardKV) sendShard(shard int, cfg int) {
	...
	for {
		...
		if !kv.leader || !kv.shardInDB(shard) || kv.cfg.Num > cfg {
			kv.mu.Unlock()
			return
		}
