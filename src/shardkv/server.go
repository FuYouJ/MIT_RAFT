...
const (
	...
	newSend         = "newSend"
	newLeader       = "newLeader"
	Config_Get_Time = time.Duration(88) * time.Millisecond
	//发送shard的等待间隔
...
type ShardKV struct {
	...
	//shard -> clerkLog
	//每一个shard独立的管理客户请求。第一个map,shard到客户的映射，第二个map，客户到客户指令的映射。
	shardClerkLog map[int]map[int64]int
	//shard -> kvDB shard 到数据库的映射 数据库是一个map嘛
	shardKvDB map[int]map[string]string
	//用于获取配置的shardMaster
	...
	shardToSend []int
	config      shardmaster.Config
	//判断本 server 是不是 leader
	leader    bool
	persister *raft.Persister
	//等待本配置发送shard/接收shard完成
...
func (kv *ShardKV) Get(args *GetArgs, reply *GetReply) {
	...
	kv.mu.Lock()
	if val, ok := kv.shardKvDB[args.Shard][args.Key]; ok {
		reply.Value = val
		reply.Err = OK
	} else {
		reply.Value = ""
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int, gid int, masters []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *ShardKV {
	...
	kv.shardClerkLog = make(map[int]map[int64]int)
	kv.shardKvDB = make(map[int]map[string]string)
	kv.shards = make(map[int]struct{})

	kv.config = shardmaster.Config{0, [10]int{}, make(map[int][]string)}
		...
		case msg := <-kv.applyCh:
			...
			case newSend:
				if !kv.haveShard(op.Shard) && kv.shardInHand(op.Shard) {
					delete(kv.shardKvDB, op.Shard)
					delete(kv.shardClerkLog, op.Shard)
			...
			case newConfig:
				...
				nc := op.NewConfig
				//防止一个cfg更新两次
				if nc.Config.Num > kv.config.Num {
					...
					if nc.Config.Num == 1 {
						...
						for shard, _ := range kv.shards {
							kv.shardKvDB[shard] = make(map[string]string)
							kv.shardClerkLog[shard] = make(map[int64]int)
						}
					}
				}
			case newShard:
				if !kv.haveShard(op.Shard) {
					...
					//防止执行两次
					ns := op.NewShards
					kv.shardKvDB[op.Shard] = make(map[string]string)
					kv.shardClerkLog[op.Shard] = make(map[int64]int)
					kv.copyDB(ns.ShardDB, kv.shardKvDB[op.Shard])
					kv.copyClerk(ns.ClerkLog, kv.shardClerkLog[op.Shard])
					kv.waitUntilMoveDone--
				}
			case request:
				//raft.ShardInfo.Printf("GID:%d cfg:%2d leader:%6v | Shard:%2d have{%v}\n", kv.gid, kv.cfg.Num, kv.leader, op.Shard, kv.shards)
				if ind, ok := kv.shardClerkLog[op.Shard][op.Clerk]; ok && ind >= op.CmdIndex {
					//如果clerk存在，并且该指令已经执行，啥也不做
					raft.ShardInfo.Printf("GID:%2d me:%2d cfg:%2d leader:%6v|op{%v}have done! index:%4d \n", kv.gid, kv.me, kv.config.Num, kv.leader, op, index)
					...
					kv.shardClerkLog[op.Shard][op.Clerk] = op.CmdIndex
					switch op.Operation {
					case putOp:
						kv.shardKvDB[op.Shard][op.Key] = op.Value
					case appendOp:
						if _, ok := kv.shardKvDB[op.Shard][op.Key]; ok {
							kv.shardKvDB[op.Shard][op.Key] = kv.shardKvDB[op.Shard][op.Key] + op.Value
						} else {
							kv.shardKvDB[op.Shard][op.Key] = op.Value
						}
					case getOp:
...
func (kv *ShardKV) decodedSnapshot(data []byte) {
	...
	var shardClerkLog map[int]map[int64]int
	var config shardmaster.Config
	if dec.Decode(&db) != nil || dec.Decode(&shardClerkLog) != nil || dec.Decode(&config) != nil {
		raft.ShardInfo.Printf("GID:%2d me:%2d leader:%6v| KV Failed to recover by snapshot!\n", kv.gid, kv.me, kv.leader)
	} else {
		kv.shardKvDB = db
		kv.shardClerkLog = shardClerkLog
		kv.config = config
...
//获取配置信息
func (kv *ShardKV) parseConfig() {
	//从config中生成待发送列表、待接收列表
	//凡是我有的且不属于我的，都发送出去
	shards := make(map[int]struct{})
	newAdd := make(map[int]struct{})
	for groupId, shard := range kv.config.Shards {
		if groupId == kv.gid {
			shards[shard] = struct{}{}
...
//判断集群是否拥有 这个shard 数据在这里
func (kv *ShardKV) shardInHand(shard int) bool {
	//判断这个shard收到没有
	_, ok := kv.shardKvDB[shard]
	return ok
}
...
func (kv *ShardKV) getConfig() {
	...
	//按序执行配置更新
	cfg = kv.sm.Query(kv.config.Num + 1)
	for {
		if !kv.leader || cfg.Num <= kv.config.Num {
			//最新，返回
			return
		}
		//配置更新
		//raft.ShardInfo.Printf("GID:%2d me:%2d cfg:%2d leader:%6v| Update config to %d\n", kv.gid, kv.me, kv.config.Num, kv.leader, cfg.Num)
...
func (kv *ShardKV) broadShard() {
	for _, shard := range kv.shardToSend {
		go kv.sendShard(shard, kv.config.Num)
	}
	raft.ShardInfo.Printf("GID:%2d me:%2d cfg:%2d leader:%6v|Begin to transfer {%v}\n", kv.gid, kv.me, kv.config.Num, kv.leader, kv.shardToSend)
}
func (kv *ShardKV) sendShard(shard int, cfgNum int) {
	...
	for {
		...
		if !kv.leader || !kv.shardInHand(shard) || kv.config.Num > cfgNum {
			kv.mu.Unlock()
			return
		}
		...
		clerkLog := make(map[int64]int)
		kv.copyDB(kv.shardKvDB[shard], kvDB)
		kv.copyClerk(kv.shardClerkLog[shard], clerkLog)

		args := PushShardArgs{cfgNum, shard, kvDB, clerkLog, kv.gid}

		if servers, ok := kv.config.Groups[groupId]; ok {
			// try each server for the shard.
			raft.ShardInfo.Printf("GID:%2d me:%2d cfg:%2d leader:%6v| transfer shard %2d to gid:%2d args{%v}\n", kv.gid, kv.me, kv.config.Num, kv.leader, shard, groupId, args)
...
func (kv *ShardKV) copyClerk(src map[int64]int, des map[int64]int) {
	for k, v := range src {
		des[k] = v
	}
}
...
func (kv *ShardKV) checkState(index int, term int) {
	...
	if kv.persister.RaftStateSize() < kv.maxraftstate*3/4.0 {
		return
	}
...
func (kv *ShardKV) encodeSnapshot() []byte {
	...
	enc.Encode(kv.shardKvDB)
	enc.Encode(kv.shardClerkLog)
...
func (kv *ShardKV) PushShard(args *PushShardArgs, reply *PushShardReply) {
	...
	raft.ShardInfo.Printf("GID:%2d me:%2d cfg:%2d leader:%6v| receive shard %2d from gid:%2d cfg:%2d need{%v} args{%v}\n", kv.gid, kv.me, kv.config.Num, kv.leader, args.Shard, args.GID, args.CfgNum, kv.needShard(), args)

	if kv.leader && args.CfgNum < kv.config.Num {
		//收到旧配置发来的shard
		//返回“接收成功”
	...
	if !kv.leader || args.CfgNum > kv.config.Num || !kv.haveShard(args.Shard) {
